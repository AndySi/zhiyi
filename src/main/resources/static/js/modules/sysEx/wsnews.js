...
layui.use(['table', 'form', 'laytpl', 'upload', 'layedit', 'layer'], function () {
    ...
    layedit.set({
        uploadImage: {
            url: baseURL + 'sysWs/wsnews/uploadCover' //接口url
            , type: 'post' //默认post
        }
    });
    ...
    upload.render({
        elem: '#test10'
        , url: baseURL + 'sysWs/wsnews/uploadCover'
    ...
    form.on('submit(btn-ok)', function (data) {
        var url = vm.itemInfo.id == null ? baseURL + "sysWs/wsnews/save" : baseURL + "sysWs/wsnews/update";
    ...
    var $ = layui.$, active = {
        ...
        update: function () {    //修改
            ...
            if (row) {
                $.get(baseURL + "sysWs/wsnews/info/" + row.id, function (r) {
        ...
        delete: function () {
            ...
            if (rows.length > 0) {
                ...
                fn_confirm('确定要删除选中的记录？', function () {
                    $.ajax({
                        type: "POST",
                        url: baseURL + "sysWs/wsnews/delete",
                        dataType: "json",
...
var vm = new Vue({
    ...
    methods: {
        ...
        options: function () {
            $.getJSON(baseURL + 'sysWs/wsnewstype/queryAllList', function (r) {
                vm.typeList = r.data;
            });
        }
    },
