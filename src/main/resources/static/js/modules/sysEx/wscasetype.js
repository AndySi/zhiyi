layui.use(['table', 'form', 'laytpl', 'upload'], function () {
    ...
    table.render({
        elem: '#layui-grid'
        , url: baseURL+'sysWs/wscasetype/list'
        , cols: [[
    ...
    form.on('submit(btn-ok)', function () {
        var url = vm.item.id == null ? baseURL+"sysWs/wscasetype/save" : baseURL+"sysWs/wscasetype/update";
    ...
    var $ = layui.$, active = {
        ...
        update: function () {    //修改
            ...
            if (row) {
                $.get(baseURL+"sysWs/wscasetype/info/" + row.id, function (r) {
                    if (r.code == 0) {
                        vm.showList = false;
                        vm.title = "修改";
                        vm.item = r.data;
        ...
        delete: function () {
            ...
            if (rows.length > 0) {
                ...
                fn_confirm('确定要删除选中的记录？', function () {
                    $.ajax({
                        type: "POST",
                        url: baseURL+"sysWs/wscasetype/delete",
                        dataType: "json",
